# -*- coding: utf-8 -*-
"""Build the Dental (DN) section block for products.js from out_*.json
(page objects: {page, header, subtitle, items:[{sku,fig,desc,box}]}).
Categories derived from each page's banner header. Writes block.txt.
Usage: python build_dental.py <CODE> "<Title>" <json_dir>
"""
...
def nice(s):
    s = clean(s)
    return s.title() if s and (s.isupper() or s.istitle() is False and s == s.upper()) else s
def cat_of(header):
    h = clean(header)
    h = re.split(r"[(,/]", h)[0].strip()   # first clause
...
for p in pages:
    ...
    if ct:
        last_key = key
    sub = clean(p.get("subtitle", ""))
    name_default = sub if sub else ct
    if name_default.isupper():
        name_default = name_default.title()
    for it in p.get("items", []):
        sku = clean(it.get("sku", "")).upper()
        if not sku or sku in seen:
            continue
        seen.add(sku)
        fig = clean(it.get("fig", ""))
        dsc = clean(it.get("desc", ""))
        desc = " — ".join(x for x in [fig, dsc] if x)
        items.append([sku, name_default or "Dental Instrument", desc, "N/A", key])
...
print("cats:", [(k, cc[k]) for k, _ in cats_used][:40])
